/*
	Tests the division operation of our array
	Author: Urs Hofmann
...
int main()
{
	...
	{
		printf("something went wrong during shallow division\n");
		throw "InvalidValue";
	}
	...
	if (!isSame)
	{
		printf("Division using operator somehow went wrong\n");
		throw "InvalidValue";
	}

	// test addition using simple assignment operator
	volB = volA / 2.345f;
	const float invVal = 1 / 2.345f;
	for (std::size_t idx = 0; idx < nElements; idx++)
	{

	 	if (volB[idx] != (volA[idx] * invVal))
			isSame = 0;
	}

	if (!isSame)
	{
		printf("Division after assignment operator somehow went wrong\n");
		throw "InvalidValue";
	}
