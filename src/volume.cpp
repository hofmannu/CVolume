...
float& volume::operator[](const std::size_t idx) { return data[idx]; }

// this version is used to get a value (quite a constant thing to do)
float volume::operator[](const std::size_t idx) const { return data[idx]; }

float volume::operator()(const std::size_t ix, const std::size_t iy, const std::size_t iz) {
...
void volume::set_dim(const std::size_t* _dim) { set_dim(_dim[0], _dim[1], _dim[2]); }
...
uint64_t volume::get_dim(const std::size_t _dim) const { return dim[_dim]; }
...
void volume::set_origin(const std::size_t _dim, const float _origin) { origin[_dim] = _origin; }
...
void volume::set_value(const std::size_t x0,
                       const std::size_t x1,
                       const std::size_t x2,
                       const float value) {
...
void volume::set_value(const std::size_t iElem, const float value) { data[iElem] = value; }
...
float volume::get_pos0(const std::size_t idx0) const { return get_pos(idx0, 0); }

float volume::get_pos1(const std::size_t idx1) const { return get_pos(idx1, 1); }

float volume::get_pos2(const std::size_t idx2) const { return get_pos(idx2, 2); }
...
std::size_t volume::get_nElements() const { return dim[0] * dim[1] * dim[2]; }
...
void volume::save_h5(const std::string& _filePath) const {
  ...
  H5::DataSet resDataset = file.createDataSet("dr", H5::PredType::NATIVE_FLOAT, mspaceRes);
  resDataset.write(res, H5::PredType::NATIVE_FLOAT);
  resDataset.close();

  // write origin to file
  H5::DataSpace mspaceOrigin(1, &col_dims);
  H5::DataSet originDataset =
      file.createDataSet("origin", H5::PredType::NATIVE_FLOAT, mspaceOrigin);
  originDataset.write(origin, H5::PredType::NATIVE_FLOAT);
  originDataset.close();

  // write dimension to file
  H5::DataSpace mspaceDim(1, &col_dims);
  H5::DataSet dimDataset = file.createDataSet("dim", H5::PredType::NATIVE_UINT, mspaceDim);
  ...
  H5::DataSet dataDataset = file.createDataSet("vol", H5::PredType::NATIVE_FLOAT, mspaceData);
...
float volume::getRangeLimitedPos(const float pos, const uint8_t _dim) const {
  float posOut = pos;
  if (posOut < get_minPos(_dim)) posOut = get_minPos(_dim);

  if (posOut > get_maxPos(_dim)) posOut = get_maxPos(_dim);

  return posOut;
}

// get cropped volume, start and stopped passed as array
void volume::get_croppedVolume(float* vol,
...
void volume::getCroppedVolume(float* vol, // array pointing to output volume
                              const uint64_t start0,
                              const uint64_t stop0,
                              const uint64_t start1,
                              const uint64_t stop1,
                              const uint64_t start2,
                              const uint64_t stop2) const {
  ...
  if (stopTrue0 != stop0) printf("Cropping not doable in dim0\n");

  const uint64_t stopTrue1 = (stop1 >= dim[1]) ? (dim[1] - 1) : stop1;
  if (stopTrue1 != stop1) printf("Cropping not doable in dim1\n");

  const uint64_t stopTrue2 = (stop2 >= dim[2]) ? (dim[2] - 1) : stop2;
  if (stopTrue2 != stop2) printf("Cropping not doable in dim2\n");
...
void rangeMinMax(const float* data,
  ...
  *localMax = data[stopIdx];
  ...
  for (std::size_t idx = startIdx; idx <= stopIdx; idx++) {
    if (*localMin > data[idx]) *localMin = data[idx];

    if (*localMax < data[idx]) *localMax = data[idx];
  }
}
...
void volume::calcMinMax() {
  ...
  for (uint8_t iProcessor = 0; iProcessor < processor_count; iProcessor++) {
    if (minVal > localMin[iProcessor]) minVal = localMin[iProcessor];

    if (maxVal < localMax[iProcessor]) maxVal = localMax[iProcessor];
  }
...
#pragma unroll
      for (uint64_t iZ = my_data->cropZ[0]; iZ < my_data->cropZ[1]; iZ++) {
        ...
        if (currVal > my_data->ptrMipZ[idxZ]) my_data->ptrMipZ[idxZ] = currVal;

        // update x mip
        const uint64_t idxX = iZ + iY * my_data->dim[0];
        if (currVal > my_data->ptrMipX[idxX]) my_data->ptrMipX[idxX] = currVal;

        // update y mip
        const uint64_t idxY = iX + my_data->dim[1] * iZ;
        if (currVal > my_data->ptrMipY[idxY]) my_data->ptrMipY[idxY] = currVal;
      }
    }
  }
  pthread_exit(NULL);
...
void volume::calcCroppedMips() {
  ...
  const uint64_t idxCropRange[6] = {get_idx0(cropRange[0]),
  ...
  const uint64_t startIdx = idxCropRange[0] + dim[0] * (idxCropRange[2] + dim[1] * idxCropRange[4]);
...
float* volume::get_croppedMipX() { return croppedMipX.data(); }

float* volume::get_croppedMipY() { return croppedMipY.data(); }

float* volume::get_croppedMipZ() { return croppedMipZ.data(); }
...
float* volume::get_croppedMipX(const float* _cropX) {
  ...
  if (flagChanged) calcCroppedMips();
...
float volume::get_norm() const { return getNorm(data.data(), nElements); }

// subroutines to fill volume with random numbers
void volume::fill_rand() { fill_rand(0.0f, 1.0f); }

// fills volume with random values between 0 and maxVal
void volume::fill_rand(const float maxVal) { fill_rand(0.0f, maxVal); }
